...
class Microphone(AudioSource):
    ...
    This will throw an ``AttributeError`` if you don't have sounddevice 0.2.11 or later installed.

    If ``device_index`` is unspecified or ``None``, the default microphone is used as the audio source. Otherwise, ``device_index`` should be the index of the device to use for audio input.

    A device index is an integer between 0 and ``sounddevice.get_device_count() - 1`` (assume we have used ``import sounddevice`` beforehand) inclusive. It represents an audio device such as a microphone or speaker. See the `sounddevice documentation <http://people.csail.mit.edu/hubert/sounddevice/docs/>`__ for more details.
    ...
    def __init__(self, device_index=None, sample_rate=None, chunk_size=1024):
        ...
        audio = self.sounddevice_module.sounddevice()
        try:
            ...
            if sample_rate is None:  # automatically set the sample rate to the hardware's default sample rate if not specified
                ...
                assert isinstance(device_info.get("defaultSampleRate"), (float, int)) and device_info["defaultSampleRate"] > 0, "Invalid device info returned from sounddevice: {}".format(device_info)
        ...
        self.format = self.sounddevice_module.paInt16  # 16-bit int sampling
        self.SAMPLE_WIDTH = self.sounddevice_module.get_sample_size(self.format)  # size of each sample
    ...
    def get_sounddevice():
        """
        Imports the sounddevice module and checks its version. Throws exceptions if sounddevice can't be found or a wrong version is installed
        """
        try:
            import sounddevice
        except ImportError:
        ...
        if LooseVersion(sounddevice.__version__) < LooseVersion("0.2.11"):
            raise AttributeError("sounddevice 0.2.11 or later is required (found version {})".format(sounddevice.__version__))
        return sounddevice
    ...
    def list_microphone_names():
        ...
        """
        audio = Microphone.get_sounddevice().sounddevice()
        try:
            result = []
    ...
    def __enter__(self):
        ...
        self.audio = self.sounddevice_module.sounddevice()
        try:
    ...
    class MicrophoneStream(object):
        ...
        def read(self, size):
            return self.sounddevice_stream.read(size, exception_on_overflow=False)

        def close(self):
            try:
                # sometimes, if the stream isn't stopped, closing the stream throws an exception
                if not self.sounddevice_stream.is_stopped():
                    self.sounddevice_stream.stop_stream()
            finally:
